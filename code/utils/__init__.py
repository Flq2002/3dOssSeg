...
def read_data_coarse(data_id, task, full_path, norm_cfg = None, device = 'cuda:0'):
    """
    coarse labels are predicted by other models
    """
    config = Config(task)
    im_path = os.path.join(config.save_dir, 'npy', f'{data_id}_image.npy')
    cl_path = os.path.join(full_path, 'predictions', f'{data_id}.nii.gz')
    lb_path = os.path.join(config.save_dir, 'npy', f'{data_id}_label.npy')
    # cl_path = os.path.join(full_path, f'{data_id}_coarse_label.nii.gz')
...
def fetch_data_coarse(batch, labeled=True):
    image = batch['image'].cuda()
    label = batch['label'].cuda().unsqueeze(1)
    coarse_label = batch['coarse_label'].cuda().unsqueeze(1)
    return image, label, coarse_label

def fetch_data_oss(batch, coarse=True):
    image = batch['image'].cuda()
    label = batch['label'].cuda()
    if coarse:

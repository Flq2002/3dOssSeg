...
def augment_rotation_coarse(data, seg, cseg, patch_size, patch_center_dist_from_border=30,
                     angle_x=(-15. / 360 * 2. * np.pi, 15. / 360 * 2. * np.pi),
                     border_mode_data='constant', border_cval_data=0, order_data=3,
                     border_mode_seg='constant', border_cval_seg=0, order_seg=1, random_crop=False):
    dim = len(patch_size)
    ...
    if seg is not None:
        if dim == 2:
            seg_result = np.zeros((seg.shape[0], seg.shape[1], patch_size[0], patch_size[1]), dtype=np.float32)
            cseg_result = np.zeros((cseg.shape[0], cseg.shape[1], patch_size[0], patch_size[1]), dtype=np.float32)
        else:
            seg_result = np.zeros((seg.shape[0], seg.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                                  dtype=np.float32)
            cseg_result = np.zeros((cseg.shape[0], cseg.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                                  dtype=np.float32)
        ...
        if seg is not None:
            for channel_id in range(seg.shape[1]):
                seg_result[sample_id, channel_id] = interpolate_img(seg[sample_id, channel_id], coords, order_seg,
                                                                    border_mode_seg, cval=border_cval_seg,
                                                                    is_seg=True)
                cseg_result[sample_id, channel_id] = interpolate_img(cseg[sample_id, channel_id], coords, order_seg,
                                                                    border_mode_seg, cval=border_cval_seg,
                                                                    is_seg=True)
    return data_result, seg_result, cseg_result
...
class SimpltSelect(Compose):
    def __init__(self, transforms):
        super(SimpltSelect, self).__init__(transforms)
        self.transforms = transforms

    def __call__(self, data_dict):

        tr_transforms = self.transforms
        for t in tr_transforms:
            data_dict = t(**data_dict)
...
class RotationTransform_norm(AbstractTransform):
    def __init__(self, norm_cfg, data_key="data", label_key="seg", p_per_sample=1.0, coarse_key = None):
        self.data_key = data_key
        self.label_key = label_key
        self.coarse_key = coarse_key
        self.p_per_sample = p_per_sample
        self.norm_cfg = norm_cfg # just one channel
    def __call__(self, **data_dict):
        data = data_dict[self.data_key]
        label = data_dict[self.label_key]
        if self.coarse_key is not None:
            coarse_label = data_dict[self.coarse_key]
        data = (data - self.norm_cfg['mean'])/self.norm_cfg['std']
        for b in range(data.shape[0]):
            if random.random() < self.p_per_sample:
                if self.coarse_key is not None:
                    data, label, coarse_label = augment_rotation_coarse(data, label, coarse_label, patch_size=data.shape[2:5])
                else:
                    data, label = augment_rotation(data, label, patch_size=data.shape[2:5])
        data_dict[self.data_key] = data
        data_dict[self.label_key] = label
        if self.coarse_key is not None:
            data_dict[self.coarse_key] = coarse_label
        return data_dict
...
class RandomCrop_coarse(AbstractTransform):
    """
    Crop randomly the image in a sample
    Args:
    output_size (int): Desired output size
    ...
    def __call__(self, **data_dict):
        image, label, coarse_label = data_dict['image'], data_dict['label'], data_dict['coarse_label']
        if label.shape[0] <= self.output_size[0] or label.shape[1] <= self.output_size[1] or label.shape[2] <= \
                self.output_size[2]:
            pw = max((self.output_size[0] - label.shape[0]) // 2 + 1, 0)
            ph = max((self.output_size[1] - label.shape[1]) // 2 + 1, 0)
        ...
        label = label[w1:w1 + self.output_size[0], h1:h1 + self.output_size[1], d1:d1 + self.output_size[2]]
        image = image[w1:w1 + self.output_size[0], h1:h1 + self.output_size[1], d1:d1 + self.output_size[2]]
        coarse_label = coarse_label[np.newaxis, np.newaxis, ...]
        label = label[np.newaxis, np.newaxis, ...]
        image = image[np.newaxis, np.newaxis, ...]
...
def AugRefiner(patch_size, norm_cfg, p_per_sample=0.5):
    tr_transforms = []
    # tr_transforms.append(Resize(patch_size))
    tr_transforms.append(RandomCrop(patch_size))
    tr_transforms.append(RotationTransform_norm(norm_cfg,
                                                "image","label",
                                                  p_per_sample=p_per_sample))
    tr_transforms.append(NumpyToTensor(['image', 'label'], 'float'))
    sample_select = SimpltSelect(tr_transforms)

    return sample_select

def AugRefiner_coarse(patch_size, norm_cfg, p_per_sample=0.5):
    tr_transforms = []
    if patch_size is not None:
        tr_transforms.append(Resize(patch_size))
    # tr_transforms.append(RandomCrop_coarse(patch_size))
    tr_transforms.append(RotationTransform_norm(norm_cfg,
                                                "image","label",
                                                  p_per_sample=p_per_sample,
                                                  coarse_key='coarse_label'))
    tr_transforms.append(NumpyToTensor(['image', 'label', 'coarse_label'], 'float'))
    sample_select = SimpltSelect(tr_transforms)

    return sample_select

def get_StrongAug_orig(patch_size, norm_cfg=None, p_per_sample=0.3, sample_num = 3):
    tr_transforms = []
    ...
    if norm_cfg is None:
        ...
        tr_transforms_select.append(RotationTransform_norm(p_per_sample=p_per_sample ,norm_cfg = norm_cfg))
...
def get_StrongAug(patch_size, norm_cfg=None, p_per_sample=0.3, sample_num = 3):
    tr_transforms = []
    tr_transforms_select = []
    # tr_transforms.append(RandomCrop(patch_size))
    tr_transforms.append(Resize(patch_size))
...
def transform_liver_train(patch_size,p_per_sample=0.5):
    tr_transforms =transforms.Compose([   
            # transforms.ScaleIntensityRanged(keys=["label"]),
            transforms.CropForegroundd(keys=["image", "label"], source_key="image"),

            transforms.RandSpatialCropd(keys=["image", "label"], roi_size=patch_size,
                                        random_size=False),
...
def transform_liver_eval(patch_size):
    trans = [
